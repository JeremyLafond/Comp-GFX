 ...
 *
 * @author Alfredo Rivero + Jeremy Lafond
 * @this {LoadedOBJ}
 */
class LoadedOBJ extends Geometry 
{
   ...
   */
  constructor(centerX, centerY, centerZ, rV, gV, bV, objStr, objM)
  {
    super();
    ...
    var objMesh = new OBJ.Mesh(this.objStr);
    ...
    this.transAndScaleVal = this.addVertexPoints(objMesh.indices, objMesh.vertices);
   ...
   * axis (indices: 0, 1, 2)
   */
  moveOBJToCenterVal(x, y, z) 
  {
    this.transMat.setTranslate(x, y, z);
  }
  ...
  setPts()
  {
    ...
    {
      pointV.push(this.vertices[i].points);
    }
    for(var k = 0; k < pointV.length; k++)
    {
      morePts.push(pointV[k].elements[0]);
      morePts.push(pointV[k].elements[1]);
      morePts.push(pointV[k].elements[2]);
    }
    this.vertices = morePts;
    return morePts;
  }
  ...
  setUV()
  {
    ...
    for(var i = 0; i < this.vertices.length; i++)
    {
      UV.push(this.vertices[i].uv[0]);
      UV.push(this.vertices[i].uv[1]);
    }
    return UV;
  }
  ...
  setNorms()
  {
    ...
    for(var i = 0; i < this.vertices.length; i++)
    {
      normN.push(this.vertices[i].normal);
    }
    for(var k = 0; k < normN.length; k++)
    {
      moreNorms.push(normN[k].elements[0]);
      moreNorms.push(normN[k].elements[1]);
      moreNorms.push(normN[k].elements[2]);
    }
    return moreNorms;
  }
...
/*  render()
  ...
  {
    this.angle++;
    this.transMat1.setTranslate(this.centerX,
                                this.centerY + (0.1 * Math.sin((this.angle + 5)/20)),
                                this.centerZ);
    this.modelMatrix = this.transMat1.multiply(this.modelMatrix);
    this.spinMat.setRotate(this.angle, 0, 1, 0);
    this.modelMatrix = this.spinMat.multiply(this.modelMatrix);
    this.scaleOBJToFitOnScreen(15);
    this.transMat1.setTranslate(this.centerX,
                                this.centerY + (0.1 * Math.sin((this.angle + 5)/20)),
                                this.centerZ);
    this.modelMatrix = this.transMat1.multiply(this.modelMatrix);
